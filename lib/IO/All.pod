...
C<getline>, C<getlines>, C<gid>, C<glob>, C<glob_raw>, C<handle>, C<head>,
...
C<lock>, C<mailer>, C<mailto>, C<mkdir>, C<mkpath>, C<mldbm>, C<mode>,
C<modes>, C<mtime>, C<name>, C<new>, C<next>, C<nlink>, C<open>,
...
Returns strings (filenames) for the files matching the glob in the IO::All::Dir.  For
example:


    io->dir($ENV{HOME})->glob_raw('*.txt')
